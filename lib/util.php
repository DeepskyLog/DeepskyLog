...
{ public function hourminuteTimeToValue($thetime)
 ...
 public  function csvObjects($result)  // Creates a csv file from an array of objects
  ...
  { global $dateformat,$baseURL,$instDir,$objObserver,$loggedUser,$objLocation,$objInstrument,$objPresentations;
    ...
    $bottom = 40;$bottomsection = 30;$top = 550;$header = 570;
    $footer = 10;$xleft = 20;$xmid = 431;$fontSizeSection = 10;$fontSizeText = 8;
		$descriptionLeadingSpace = 20;$sectionBarSpace = 3;
    $deltalineSection = 2;    
  ...
  { global $objReportLayout,$dateformat,$baseURL,$instDir,$objObserver,$loggedUser,$objLocation,$objInstrument,$objPresentations;
    ...
    if($sort=='objectconstellation') $sort='con'; else $sort=''; $indexlist=array();
  ...
  { global $loggedUser, $dateformat, $instDir, $objObservation, $objObserver, $objInstrument, $objLocation, $objPresentations, $objObject, $objFilter, $objEyepiece, $objLens;
    ...
    { if($i++>0)
      ...
      if($obs['eyepieceid']) $pdf->ezTable($tmp,     array("eyepiece" => "test"),   "", array("width" => "500", "showHeadings" => "0", "showLines" => "0", "shaded" => "0"));
      $tmp=array(array("filter"=>$temp["filter"]));
      if($obs['filterid'])   $pdf->ezTable($tmp,         array("filter" => "test"),     "", array("width" => "500", "showHeadings" => "0", "showLines" => "0", "shaded" => "0"));
      $tmp=array(array("lens"=>$temp["lens"]));
      if($obs['lensid'])     $pdf->ezTable($tmp,             array("lens" => "test"),       "", array("width" => "500", "showHeadings" => "0", "showLines" => "0", "shaded" => "0"));
      $tmp=array(array("seeing"=>$temp["seeing"]));
      if($seeing)     $pdf->ezTable($tmp,         array("seeing" => "test"),     "", array("width" => "500", "showHeadings" => "0", "showLines" => "0", "shaded" => "0"));
      $tmp=array(array("limmag"=>$temp["limmag"]));
      if($obs['limmag'])     $pdf->ezTable($tmp,         array("limmag" => "test"),     "", array("width" => "500", "showHeadings" => "0", "showLines" => "0", "shaded" => "0"));
      $tmp=array(array("visibility"=>$temp["visibility"]));
      if($obs['visibility']) $pdf->ezTable($tmp, array("visibility" => "test"), "", array("width" => "500", "showHeadings" => "0", "showLines" => "0", "shaded" => "0"));
  ...
  { global $baseURL;
    ...
    { $currentpage=ceil($min/$step)+1;
		  ...
		  $content.= "<input type=\"number\" min=\"1\" max=\"" . $pages . "\" name=\"multiplepagenr\" size=\"3\" class=\"inputfield centered\" value=\"".$currentpage."\" />";	
  ...
  { global $baseURL;
    ...
    { $currentpage=ceil($min/$step)+1;
		  ...
		  $content.= "<input type=\"number\" min=\"1\" max=\"" . $pages . "\" name=\"viewObjectObservationsmultiplepagenr\" size=\"3\" class=\"inputfield centered\" value=\"".$currentpage."\" />";	
