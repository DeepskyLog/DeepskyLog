...
class PrintAtlas
{
    public $astroObjectsArr;
    public $atlasmagnitude = 0;
    public $atlaspagedecldeg = 0;
    public $atlaspagerahr = 0;
    public $atlaspagezoomdeg = 2;
    public $by = 0;
    public $canvasDimensionXpx;
    public $canvasDimensionYpx;
    public $canvasX1px;
    public $canvasX2px;
    public $canvasY1px;
    public $canvasY2px;
    public $conBoundries = array();
    public $coordGridsH;
    public $diam1SecToPxCt = 1;
    public $diam2SecToPxCt = 1;
    public $dsl_amn;
    public $dsl_asc;
    public $dsl_deg;
    // fn coordDeclDecToDegMin results
    public $dsl_hr;
    public $dsl_min;
    public $dsl_sec;
    // fn coordHrDecToHrMin    results
    public $Dsteps = 10;
    // Number of steps for drawing coordinate lines between major steps
    public $f12OverPi  = 3.8197186342054880584532103209403;
    public $f180OverPi = 57.295779513082320876798154814105;
    public $fPi        = 3.1415926535;
    public $fPiOver2   = 1.5707963267948966192313216916398;
    public $fPiOver12  = 0.26179938779914943653855361527329;
    public $fPiOver180 = 0.017453292519943295769236907684886;
    public $f2Pi       = 6.283185307179586476925286766559;
    public $fontSize1a = 10;
    public $fontSize1b = 6;
    public $fontSizeAck = 4;
    public $gridActualDimension = 16;
    public $gridCenterOffsetXpx;
    public $gridCenterOffsetYpx;
    public $gridD0rad;
    public $griddDdeg;
    public $gridDyRad;
    public $gridDy1rad;
    ...
    public $gridLx1rad;
    public $gridLx2rad;
    public $gridMaxDimension = 23;
    public $gridMinDimension = 0;
    public $gridOffsetXpx = 50;
    public $gridOffsetYpx = 50;
    public $gridrdDdeg;
    public $gridrdLhr;
    public $gridrLhr;
    public $gridruDdeg;
    ...
    public $griduDdeg;
    public $gridWidthXpx;
    public $gridWidthXpx2 = 0;
    public $gridHeightYpx;
    public $gridHeightYpx2 = 0;
    public $labelsArr = array();
    public $Legend1x = 25;
    public $Legend1y = 20;
    public $Legend2x = 55;
    public $Legend2y = 35;
    public $Lsteps = 10;
    public $lx = 0;
    public $maxshowndsomag = -99;
    public $minObjectSize = 2.5;
    public $nsegmente = 8;
    public $rx = 0;
    public $starsmagnitude;
    public $theItemPage;
    public $theOrientation;
    public $thePageSize;
    public $ty = 0;

    public $pdf;

    public $gridDimensions =  array(
    ...
    public function astroDrawBRTNBObject($i)
    {
        $this->gridLDrad(
    ...
    public function astroDrawCLANBObject($i)
    {
        $this->gridLDrad(
            $this->astroObjectsArr[$i]["ra"],
            $this->astroObjectsArr[$i]["decl"]
    ...
    public function astroDrawConstellations()
    {
    ...
    public function astroDrawDRKNBObject($i)
    {
    ...
    public function astroDrawGCObject($i)
    {
        $this->gridLDrad($this->astroObjectsArr[$i]["ra"], $this->astroObjectsArr[$i]["decl"]);
        $cx = $this->gridCenterOffsetXpx + $this->gridXpx($this->gridLxRad);
        $cy = $this->gridCenterOffsetYpx + $this->gridYpx($this->gridDyRad);
    ...
    public function astroDrawGXCLObject($i)
    {
        $this->gridLDrad($this->astroObjectsArr[$i]["ra"], $this->astroObjectsArr[$i]["decl"]);
        $cx = $this->gridCenterOffsetXpx + $this->gridXpx($this->gridLxRad);
        $cy = $this->gridCenterOffsetYpx + $this->gridYpx($this->gridDyRad);
    ...
    public function astroDrawGXObject($i)
    {
        $this->gridLDrad($this->astroObjectsArr[$i]["ra"], $this->astroObjectsArr[$i]["decl"]);
        $cx = $this->gridCenterOffsetXpx + $this->gridXpx($this->gridLxRad);
        $cy = $this->gridCenterOffsetYpx + $this->gridYpx($this->gridDyRad);
    ...
    public function atlasDrawLegend()
    {
        ...
        $this->pdf->addTextWrap(
            $this->Legend2x + 10,
            $this->canvasDimensionYpx - $this->Legend2y,
            $this->fontSize1b,
            'GALXY',
            30,
            'left'
    ...
    public function astroDrawObjectLabel($cx, $cy, $d, $name, $seen)
    {
    ...
    public function astroDrawOCObject($i)
    {
        $this->gridLDrad($this->astroObjectsArr[$i]["ra"], $this->astroObjectsArr[$i]["decl"]);
        $cx = $this->gridCenterOffsetXpx + $this->gridXpx($this->gridLxRad);
        $cy = $this->gridCenterOffsetYpx + $this->gridYpx($this->gridDyRad);
    ...
    public function astroDrawStarObject($i)
    ...
    public function astroDrawStar1Object($i)
    {
        ...
        $cy = $this->gridCenterOffsetYpx + $this->gridYpx($this->gridDyRad);
        if((!((($cx - $d < $this->lx) || ($cx + $d > $this->rx)))) &&
           (!((($cy + $d > $this->ty) || ($cy - $d < $this->by))))) { //$this->pdf->filledEllipse($cx,$cy,(.5*$d),(.5*$d),0,$this->nsegmente);
            //$this->astroDrawObjectLabel($cx,$cy,(($d+1)>>1),$this->astroObjectsArr[$i]["name"],$this->astroObjectsArr[$i]["seen"]);
        }
    }
    public function astroDrawStarxObject($i)
    ...
    public function astroDrawObjects($theobject = '')
    {
        ...
        for($i = 0;$i < $z;$i++) {
            ...
            if (in_array($this->astroObjectsArr[$i]["name"], $objObjectOutlines->getAllObjects())) {
            ...
            } elseif((!(in_array($thecat, $excludelist))) && ($this->astroObjectsArr[$i]["type"] != 'AASTAR1')) {
                ...
                } elseif($this->astroObjectsArr[$i]["type"] == 'AA1STAR') {
                    $this->astroDrawStar1Object($i);
                } elseif(in_array($this->astroObjectsArr[$i]["type"], array('AA3STAR','ASTAR','AA5STAR','AA6STAR','AA7STAR','AA8STAR','DS'))) {
                    $this->astroDrawStarxObject($i);
                } elseif(in_array($this->astroObjectsArr[$i]["type"], array('ASTER','LMCOC','OPNCL','SMCOC'))) {
                    $this->astroDrawOCObject($i);
                } elseif(in_array($this->astroObjectsArr[$i]["type"], array('BRTNB','EMINB','ENRNN','ENSTR','GXADN','LMCDN','REFNB','RNHII','SMCDN','SNREM','STNEB','WRNEB'))) {
                    $this->astroDrawBRTNBObject($i);
                } elseif(in_array($this->astroObjectsArr[$i]["type"], array('CLANB','GACAN','LMCCN','SMCCN','GXADN','LMCDN','HII'))) {
                    $this->astroDrawCLANBObject($i);
                } elseif(in_array($this->astroObjectsArr[$i]["type"], array('DRKNB'))) {
                    $this->astroDrawDRKNBObject($i);
                } elseif(in_array($this->astroObjectsArr[$i]["type"], array('GALCL'))) {
                    $this->astroDrawGXCLObject($i);
                } elseif(in_array($this->astroObjectsArr[$i]["type"], array('GALXY'))) {
                    $this->astroDrawGXObject($i);
                } elseif(in_array($this->astroObjectsArr[$i]["type"], array('GLOCL','GXAGC','LMCGC','SMCGC'))) {
                    $this->astroDrawGCObject($i);
                } elseif(in_array($this->astroObjectsArr[$i]["type"], array('PLNNB'))) {
                    $this->astroDrawPNObject($i);
                } elseif(in_array($this->astroObjectsArr[$i]["type"], array('QUASR'))) {
    ...
    public function astroDrawObject($theobject = '')
    {
        global $objObject,$objUtil,$objObjectOutlines;
    ...
    public function astroDrawObjectOutline($i)
    {
        global $objObject,$objUtil,$objObjectOutlines;
    ...
    public function astroDrawPNObject($i)
    {
    ...
    public function astroDrawQSRObject($i)
    {
        $this->pdf->addText(10, 10, $this->fontSize1b, "PN");
        $this->gridLDrad($this->astroObjectsArr[$i]["ra"], $this->astroObjectsArr[$i]["decl"]);
        $cx = $this->gridCenterOffsetXpx + $this->gridXpx($this->gridLxRad);
    ...
    public function astroDrawStarsArr()
    {
        global $objStar;
        for($m = 8;$m <= $this->starsmagnitude;$m++) { //if($m>$this->gridDimensions[$this->gridActualDimension][3])
    ...
    public function astroGetConstellationFromCoordinates($thera, $thedecl)
    {
        $tempdecl = -90;
        $tempcon = "";
        $thera0 = 0.0;
    ...
    public function canvasDrawStar($i)
    {
        $name = '';
    ...
    public function coordDeclDecToDegMin($theDeg)
    {
        if($theDeg > 90) {
            $theDeg = 90;
        }
    ...
    public function coordHrDecToHrMin($theHr)
    {
        while(($theHr) > 24) {
            $theHr -= 24;
        }
    ...
    public function coordHrDecToHrMinSec($theHr)
    {
        while(($theHr) > 24) {
            $theHr -= 24;
        }
        ...
        } elseif($this->dsl_min > 0) {
            return $this->dsl_hr.'h'.$this->dsl_min.'m';
        }
        return $this->dsl_hr.'h';
    }

    public function coordGridLxDyToString()
    {
    ...
    { //jg.setFont("Lucida Console", fontSize1a."px", Font.PLAIN);
        ...
        $this->gridLDinvRad($this->gridOffsetXpx + (((int)$this->gridWidthXpx + 1) >> 1), $this->gridOffsetYpx);
        $this->griduDdeg = $this->gridDyRad * $this->f180OverPi;
        $this->gridLDinvRad($this->gridOffsetXpx + (((int)$this->gridWidthXpx + 1) >> 1), $this->gridOffsetYpx + $this->gridHeightYpx);
        ...
        if((($this->gridD0rad + $this->gridSpanDrad) < ($this->fPiOver2)) && (($this->gridD0rad - $this->gridSpanDrad) > -($this->fPiOver2))) {
            ...
            $griduDdeg = $Udeg;
            $griddDdeg = $Ddeg;
        } elseif(($this->gridD0rad + $this->gridSpanDrad) >= ($this->fPiOver2)) {
            $Lhr = 24;
            $RhrNeg = 0;
            $Udeg = 90;
        ...
        for($d = $DdegStart;$d <= $Udeg;$d += $DStep) {
            ...
            } elseif($this->canvasX2px && ($this->canvasY2px >= $this->gridOffsetYpx + $this->gridHeightYpx - ($this->fontSize1a))) {
            ...
            } elseif($this->canvasX2px && ($this->canvasY2px <= $this->gridOffsetYpx)) {
        ...
        if($this->gridD0rad < 0) {
            for($l = $LhrStart;$l > $RhrNeg;$l -= $LStep) {
                ...
                } elseif($this->canvasX2px && ($this->canvasX2px + 15 >= $this->gridOffsetXpx + $this->gridWidthXpx)) {
                ...
                } elseif($this->canvasY2px && ($this->canvasY2px - 15 <= $this->gridOffsetYpx)) {
                    $this->labelsArr[] = array($this->canvasX1px - 30,$this->gridOffsetYpx - 10,60,$this->fontSize1b,$this->coordHrDecToHrMin($l),'center');
                } elseif(($this->canvasX2px) && ($this->canvasY2px > ($this->canvasDimensionYpx >> 1))) {
            ...
            for($l = $LhrStart;$l > $RhrNeg;$l -= $LStep) {
                ...
                if($this->canvasX1px && ($this->canvasX1px - 15 <= $this->gridOffsetXpx)) {
                    $this->labelsArr[] = array($this->gridOffsetXpx - 64,$this->canvasY2px - ($this->fontSize1a >> 2),60,$this->fontSize1b,$this->coordHrDecToHrMin($l),'right');
                } elseif($this->canvasX1px && ($this->canvasX1px + 15 >= $this->gridOffsetXpx + $this->gridWidthXpx)) {
                    $this->labelsArr[] = array($this->gridOffsetXpx + $this->gridWidthXpx + 4,$this->canvasY2px - ($this->fontSize1a >> 2),60,$this->fontSize1b,$this->coordHrDecToHrMin($l),'left');
                } elseif(($this->canvasX1px) && ($this->canvasY2px > ((int)$this->canvasDimensionYpx >> 1))) {
    ...
    public function gridDrawLineLD($Lhr1, $Ddeg1, $Lhr2, $Ddeg2)
    {
        $this->gridLDrad($Lhr1, $Ddeg1);
        $x1 = $this->gridLxRad;
        $y1 = $this->gridDyRad;
        $this->gridLDrad($Lhr2, $Ddeg2);
        $x2 = $this->gridLxRad;
        $y2 = $this->gridDyRad;
        if(($x1 < -($this->gridSpanLrad)) && ($x2 < -($this->gridSpanLrad))) {
            return 0;
        }
        if(($x1 > $this->gridSpanLrad) && ($x2 > $this->gridSpanLrad)) {
            return 0;
        }
        if(($y1 < -($this->gridSpanDrad)) && ($y2 < -($this->gridSpanDrad))) {
            return 0;
        }
        if(($y1 > $this->gridSpanDrad) && ($y2 > $this->gridSpanDrad)) {
            return 0;
        }
        if($x1 < -($this->gridSpanLrad)) {
            if($x2 == $x1) {
                return 0;
            } else {
                $y1 = (((-($this->gridSpanLrad) - $x1) / ($x2 - $x1)) * ($y2 - $y1)) + $y1;
                $x1 = -($this->gridSpanLrad);
            }
        }
        if($x1 > ($this->gridSpanLrad)) {
            if($x2 == $x1) {
        ...
        if($y1 > ($this->gridSpanDrad)) {
            if($y2 == $y1) {
                return 0;
            } else {
                $x1 = (((($this->gridSpanDrad) - $y1) / ($y2 - $y1)) * ($x2 - $x1)) + $x1;
                $y1 = ($this->gridSpanDrad);
            }
        }
        if($y1 < -($this->gridSpanDrad)) {
            if($y2 == $y1) {
        ...
        if(($y1 < -($this->gridSpanDrad)) || ($y1 > ($this->gridSpanDrad)) || ($x1 < -($this->gridSpanLrad)) || ($x1 > ($this->gridSpanLrad))) {
            return 0;
        }
        if($x2 < -($this->gridSpanLrad)) {
            if($x2 == $x1) {
                return 0;
            } else {
                $y2 = (((-($this->gridSpanLrad) - $x1) / ($x2 - $x1)) * ($y2 - $y1)) + $y1;
                $x2 = -($this->gridSpanLrad);
            }
        }
        if($x2 > ($this->gridSpanLrad)) {
            if($x2 == $x1) {
        ...
        if($y2 > ($this->gridSpanDrad)) {
            if($y2 == $y1) {
                return 0;
            } else {
                $x2 = (((($this->gridSpanDrad) - $y1) / ($y2 - $y1)) * ($x2 - $x1)) + $x1;
                $y2 = ($this->gridSpanDrad);
            }
        }
        if($y2 < -($this->gridSpanDrad)) {
            if($y2 == $y1) {
        ...
        if(($y2 < -($this->gridSpanDrad)) || ($y2 > ($this->gridSpanDrad)) || ($x2 < -($this->gridSpanLrad)) || ($x2 > ($this->gridSpanLrad))) {
            return 0;
        }
    ...
    public function gridDrawLongLineLD($Lhr1, $Ddeg1, $Lhr2, $Ddeg2)
    {
        $this->gridLDrad($Lhr1, $Ddeg1);
        $x1 = $this->gridLxRad;
        $y1 = $this->gridDyRad;
        ...
        $x2 = $this->gridLxRad;
        $y2 = $this->gridDyRad;
        if((abs($x1) > $this->fPiOver2) || (abs($y1) > $this->fPiOver2) || (abs($y1) > $this->fPiOver2) || (abs($y2) > $this->fPiOver2)) {
            return 0;
        }
        if(($x1 < -$this->gridSpanLrad) && ($x2 < -$this->gridSpanLrad)) {
            return 0;
        ...
        */
        ...
        for($i = 0.0;$i < $ds;) {
            $this->gridLDrad($Lhr1 + (($Lhr2 - $Lhr1) * $i / $ds), $Ddeg1 + (($Ddeg2 - $Ddeg1) * $i / $ds));
            $x1 = $this->gridLxRad;
            $y1 = $this->gridDyRad;
            $i++;
            $this->gridLDrad($Lhr1 + (($Lhr2 - $Lhr1) * $i / $ds), $Ddeg1 + (($Ddeg2 - $Ddeg1) * $i / $ds));
    ...
    private function gridInit()
    {
        ...
        $this->gridWidthXpx2 = (((int)$this->gridWidthXpx + 1) >> 1);
        $this->gridCenterOffsetXpx = $this->gridOffsetXpx + (((int)$this->gridWidthXpx + 1) >> 1);
        $this->gridHeightYpx = $this->canvasDimensionYpx - ($this->gridOffsetYpx << 1);
        $this->gridHeightYpx2 = (((int)$this->gridHeightYpx + 1) >> 1);
        $this->gridCenterOffsetYpx = $this->gridOffsetYpx + (((int)$this->gridHeightYpx + 1) >> 1);
    ...
    private function gridInitSpecial($thex, $they, $w, $h)
    {
        $this->canvasDimensionXpx = $this->pdf->ez['pageWidth'];
        $this->canvasDimensionYpx = $this->pdf->ez['pageHeight'];
        $this->gridOffsetXpx = $thex;
        $this->gridOffsetYpx = $they;
    ...
    public function gridInitScale($gridLHr, $gridDdeg)
    {
    ...
    public function gridLDinvRad($XpxAbsScr, $YpxAbsScr)
    {
    ...
    public function gridLDrad($Lhr, $Ddeg)
    {
        $Lrad = $Lhr * $this->fPiOver12;
        $Drad = $Ddeg * $this->fPiOver180;
        if($Lrad > $this->gridL0rad + $this->fPi) {
            $Lrad = $Lrad - ($this->f2Pi);
        }
        if($Lrad < $this->gridL0rad - $this->fPi) {
            $Lrad = $Lrad + ($this->f2Pi);
    ...
    public function gridXpx($Lrad)
    {
        return (($this->gridWidthXpx2 * $Lrad / $this->gridSpanLrad));
    }

    public function gridYpx($Drad)
    {
    ...
    public function pdfAtlas($nostream = false)  // Creates a pdf atlas page
    {
        ...
        $this->pdf->rectangle(
            $this->gridOffsetXpx - 1,
            $this->gridOffsetYpx - 1,
            ($this->canvasDimensionXpx - ($this->gridOffsetXpx << 1)) + 2,
            ($this->canvasDimensionYpx - ($this->gridOffsetYpx << 1)) + 2
        );
        ...
        $atlastype = '';
        if($objUtil->checkRequestKey('atlastype', 0) == 0) {
            $atlastype = 'DSLOP';
        }
        if($objUtil->checkRequestKey('atlastype', 0) == 1) {
            $atlastype = 'DSLLP';
        }
        if($objUtil->checkRequestKey('atlastype', 0) == 2) {
            $atlastype = 'DSLDP';
        }
        if($objUtil->checkRequestKey('atlastype', 0) == 3) {
            $atlastype = 'DSLOL';
        }
        if($objUtil->checkRequestKey('atlastype', 0) == 4) {
            $atlastype = 'DSLLL';
        }
        if($objUtil->checkRequestKey('atlastype', 0) == 5) {
            $atlastype = 'DSLDL';
        }
    ...
    public function pdfAtlasObjectSet($theobject, $theShowname, $theSet, $thedsos, $thestars, $thephotos, $datapage = 'false', $reportlayoutselect = '', $ephemerides = 'true', $yearephemerides = false, $nostream = false)
    {
        ...
        if($datapage == 'true') {
            ...
            $this->pdf->addTextWrap(300, $liney, 10, _("Object magnitude").': '.($theobjectdata['objectmagnitude'] ? $theobjectdata['objectmagnitude'] : '-'), 200, 'left');
            $this->pdf->addTextWrap(550, $liney, 10, _("Constellation name").': '.$theobjectdata['objectconstellationfull'], 200, 'left');
            $liney -= 15;
            ...
            if($loggedUser) {
                ...
                if($objObserver->getObserverProperty($loggedUser, 'stdLocation') && ($yearephemerides == 'true')) {
                    ...
                    for($i = 1;$i < 7;$i++) {
                        ...
                        } elseif(($theEphemerides1[$i]['altitude'] != '-') &&
                         (($theEphemerides1[$i]['altitude'] == $theEphemerides15[$i]['altitude']) ||
                        ...
                        $this->pdf->addTextWrap(
                            ...
                            $liney,
                            8,
                            $colorclass.$this->filterdegpart($theEphemerides1[$i]['altitude']).$colorclass2,
                            50,
                            'center'
                        );
                        $colorclass = "";
                    ...
                    for($i = 1;$i < 7;$i++) {
                        ...
                        if((date("H:i", $theNightEphemerides1[$i]["astronomical_twilight_end"]) != "00:00") &&
                            $objUtil->checkNightHourMinuteBetweenOthers(
                                $theEphemerides1[$i]['transit'],
                    ...
                    for($i = 7;$i < 13;$i++) {
                     ...
                     (($theEphemerides1[$i]['altitude'] == $theEphemerides15[$i]['altitude']) ||
                        ...
                        } elseif(($theEphemerides15[$i]['altitude'] != '-') &&
                         (($theEphemerides15[$i]['altitude'] == $theEphemerides1[$i]['altitude']) ||
                              ($theEphemerides15[$i]['altitude'] == $theEphemerides1[$i + 1]['altitude']))) {
                            $colorclass = "<c:uline><b>";
                            $colorclass2 = "</c:uline></b>";
        ...
        if(is_array($thephotos) && array_key_exists(0, $thephotos) && ($thephotos[0] > 0)) {
            ...
            if($datapage != 'true') {
                ...
                $this->pdf->addTextWrap(50, $liney, 10, _("Alternative object names").": ".($theobjectdata['altname'] ? $theobjectdata['altname'] : '-'), 450, 'left');
                $liney -= 25;
                $this->pdf->addTextWrap(50, $liney, 10, _("Object type").': '.$theobjectdata['objecttypefull'], 200, 'left');
                $this->pdf->addTextWrap(300, $liney, 10, _("Object magnitude").': '.($theobjectdata['objectmagnitude'] ? $theobjectdata['objectmagnitude'] : '-'), 200, 'left');
                $liney -= 15;
                $this->pdf->addTextWrap(50, $liney, 10, _("Constellation name").': '.$theobjectdata['objectconstellationfull'], 200, 'left');
                $this->pdf->addTextWrap(300, $liney, 10, _("Object surface brightness").': '.($theobjectdata['objectsurfacebrightness'] ? $theobjectdata['objectsurfacebrightness'] : '-'), 200, 'left');
                $liney -= 15;
                $this->pdf->addTextWrap(50, $liney, 10, _("Right ascension").': '.$theobjectdata['objectrahms'], 200, 'left');
                        ...
                        if($dataelement['fieldwidth']) {
                            ...
                            if($dataelement['fieldname'] == "showname") {
                                ...
                                $this->pdf->addTextWrap(
                                    ...
                                    $y - ($deltaline * $dataelement['fieldline']),
                                    $fontSizeText,
                                    $dataelement['fieldbefore'].html_entity_decode($valueA[($loggedUser ? $objObserver->getObserverProperty($loggedUser, 'standardAtlasCode', 'urano') : 'urano')]).$dataelement['fieldafter'],
                                    $dataelement['fieldwidth'],
                                    $justification
                                );
                                $deltaymax = max($deltaymax, $dataelement['fieldline']);
                                ...
                                if($valueA[$dataelement['fieldname']] != '') {
                                    $this->pdf->addTextWrap(
                                        ...
                                        $y - ($deltaline * $dataelement['fieldline']),
                                        $fontSizeText,
                                        $dataelement['fieldbefore'].html_entity_decode($valueA[$dataelement['fieldname']]).$dataelement['fieldafter'],
                                        $dataelement['fieldwidth'],
                                        $justification
                                    );
                                    $deltaymax = max($deltaymax, $dataelement['fieldline']);
    ...
    public function roundPrecision($theValue, $thePrecision)
    {
