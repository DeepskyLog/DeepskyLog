...
{	return jg.fillEllipseObject(canvasOffsetXpx+cx-((d+1)>>1),canvasOffsetYpx+canvasDimensionYpx-cy-((d+1)>>1),d,d,name,object);
}
...
{ gridLDrad(Lhr,Ddeg); 
	...
	canvasDrawLine(x-((d1+1)>>1)-4, y, x-d1-4, y);
	canvasDrawLine(x, y-4-d2, x, y-((d2+1)>>1)-4);
  canvasDrawLine(x, y+4+d2, x, y+((d2+1)>>1)+4);
	canvasDrawLineObject(x+((d1+1)>>1)+4, y, x+4+d1, y, nameText,seen,object);
}
...
{ gridLDrad(Lhr,Ddeg); 
  ...
  canvasDrawLine(x, y+2+d2, x, y+2);
	canvasDrawLineObject(x+2, y, x+2+d1, y, nameText,seen,object);
}
