...
{ jg.setColor(starColor);
  gridDrawFilledCirclePxObject(Lhr,Ddeg,Math.max(Math.round((gridActualDimension-(0.02*vMag)-1.5)*1.5),1),name,object);
}
...
{ var ret='';
  ...
  { name=getAstroObjectValue(object,"nameBayer")+'&nbsp;'+getAstroObjectValue(object,"nameBayer2")+'&nbsp;'; 
    if(name!="&nbsp;&nbsp;") name+=getAstroObjectValue(object,"nameCon"); else name='SOA'+getAstroObjectValue(object,"nameSOA");
...
{ if(onClickHandling==true)
  ...
  y=event.clientY+1;
	...
	{ gridLDinvRad(x,y);
    atlaspagerahr=gridLxRad/3.82;
    atlaspagedecldeg=gridDyRad/(57.295);
    gridInitScale(atlaspagerahr,atlaspagedecldeg,atlaspagezoomdeg);
    atlasRedraw();
	}
...
{	return jg.fillEllipse(canvasOffsetXpx+cx-(d>>1),canvasOffsetYpx+canvasDimensionYpx-cy-(d>>1),d,d);
}
function canvasDrawFilledCircleObject(cx,cy,d,name,object)
{	return jg.fillEllipseObject(canvasOffsetXpx+cx-(d>>1),canvasOffsetYpx+canvasDimensionYpx-cy-(d>>1),d,d,name,object);
}
...
{ jg.setFont("Lucida Console", fontSize1a+"px", Font.PLAIN);
  ...
  luLrad=gridLxRad;
  gridluLhr=luLrad*3.82;
  luDrad=gridDyRad;
  gridluDdeg=luDrad*57.295;
  ...
  ruLrad=gridLxRad;
  gridruLhr=ruLrad*3.82;
  ruDrad=gridDyRad;
  gridruDdeg=ruDrad*57.295;
  ...
  ldLrad=gridLxRad;
  gridldLhr=ldLrad*3.82;
  ldDrad=gridDyRad;
  gridldDdeg=ldDrad*57.295;
  ...
  rdLrad=gridLxRad;
  gridrdLhr=rdLrad*3.82;
  rdDrad=gridDyRad;
  gridrdDdeg=rdDrad*57.295;
  ...
  griduDdeg=gridDyRad*57.295;
  gridLDinvRad(div5Left+divOffsetXpx+canvasOffsetXpx+gridOffsetXpx+gridWidthXpx,div5Top+divOffsetYpx+canvasOffsetYpx+gridOffsetYpx+gridHeightYpx);
  griddDdeg=gridDyRad*57.295;
   
  if(((gridD0rad+gridSpanDrad)<(1.57))&&((gridD0rad-gridSpanDrad)>-(1.57)))
	{	if(gridD0rad>0)
	  { Lrad=luLrad;
	    Rrad=ruLrad;
    }
  ...
  else if((gridD0rad+gridSpanDrad)>=(1.57))
	{ Lhr=gridL0rad*3.82+12+.01;
	  RhrNeg=gridL0rad*3.82-12;
		Udeg=90;
		Ddeg=Math.min(gridD0rad-gridSpanDrad,Math.min(ldDrad,rdDrad))*57.295;
	}
  else if((gridD0rad-gridSpanDrad)<=-(Math.PI/2))
	{ Lhr=gridL0rad*3.82+12+.01;
	  RhrNeg=gridL0rad*3.82-12;
		Udeg=Math.max(gridD0rad+gridSpanDrad,Math.max(luDrad,ruDrad))*57.295;
		Ddeg=-90;
	}
...
{ AngleDeg=((((AngleDeg*1.0)+90)%180)*0.01745);
  gridLDrad(Lhr,Ddeg); 
	...
	canvasDrawEllipseTilt(gridCenterOffsetXpx+gridXpx(x1),gridCenterOffsetYpx+gridYpx(y1),gridDiam1SecToPxMin(Diam1Sec),gridDiam2SecToPxMin(Diam2Sec),AngleDeg,nameText,seen);
}
function gridDrawEllipseTiltObject(Lhr,Ddeg,Diam1Sec,Diam2Sec,AngleDeg,nameText,seen,object)
{ AngleDeg=((((AngleDeg*1.0)+90)%180)*0.01745);
  gridLDrad(Lhr,Ddeg); 
	x1=gridLxRad; y1=gridDyRad;
	canvasDrawEllipseTiltObject(gridCenterOffsetXpx+gridXpx(x1),gridCenterOffsetYpx+gridYpx(y1),gridDiam1SecToPxMin(Diam1Sec),gridDiam2SecToPxMin(Diam2Sec),AngleDeg,nameText,seen,object);
}
...
{ gridLDrad(Lhr,Ddeg); 
	...
	canvasDrawLine(x-(d1>>1), y, x+(d1>>1), y);
	canvasDrawLine(x, y-(d2>>1), x, y+(d2>>1));
}
...
{ gridLDrad(Lhr,Ddeg);
  canvasDrawRectangleCWH(gridCenterOffsetXpx+gridXpx(gridLxRad),
  		                   ...
  		                   gridDiam2SecToPxMin(hSec),pa,nm,seen);
}
...
{ gridLDrad(Lhr,Ddeg);
  canvasDrawRectangleCWHObject(gridCenterOffsetXpx+gridXpx(gridLxRad),
  		                   gridCenterOffsetYpx+gridYpx(gridDyRad),
  		                   gridDiam1SecToPxMin(wSec),
  		                   gridDiam2SecToPxMin(hSec),pa,nm,seen,object);
}
...
{	gridWidthXpx=canvasDimensionXpx-gridOffsetXpx-gridOffsetXpx;
	gridWidthXpx2=(gridWidthXpx>>1);
	gridCenterOffsetXpx=gridOffsetXpx+(gridWidthXpx>>1);
	gridHeightYpx=canvasDimensionYpx-gridOffsetYpx-gridOffsetYpx;
	gridHeightYpx2=(gridHeightYpx>>1);
	gridCenterOffsetYpx=gridOffsetYpx+(gridHeightYpx>>1);
}
...
{ gridActualDimension=gridMaxDimension;
	while((gridActualDimension>gridMinDimension)&&(gridDimensions[gridActualDimension][0]<desiredScale))
	  gridActualDimension--;
	gridL0rad=gridLHr*0.2618;
	gridD0rad=gridDdeg*0.0174533;
  ...
  {	gridSpanD=gridDimensions[gridActualDimension][0]*(gridHeightYpx/gridWidthXpx);
	...
	gridSpanLrad=gridSpanL*0.0174533;
	gridSpanDrad=gridSpanD*0.0174533;
...
{ Lrad=Lhr*.2618; Drad=Ddeg*0.0174533;
	if(Lrad>gridL0rad+3.1415926535) Lrad=Lrad-(6.2832);
	if(Lrad<gridL0rad-Math.PI) Lrad=Lrad+(6.2832);
	...
	{ var sinalpha=xRad/drad;
	  var cosalpha=yRad/drad;
		...
		if(cosLacc>=0)
  		...
  		gridLxRad=gridL0rad+3.1415926535-(Math.asin(Math.sin(drad)*sinalpha/Math.sin(Dacc)));		
		gridDyRad=(1.57-Dacc);
	}
	else
	{ gridLxRad=gridL0rad;
...
{ gridInitScaleFixed(gridL0rad*3.82,gridD0rad*57.3,Math.max(Math.min(gridActualDimension+zoomFactor,gridMaxDimension),gridMinDimension));
}
function gridZoomLevel(zoomLevel)
{ gridInitScaleFixed(gridL0rad*3.82,gridD0rad*57.3,Math.max(Math.min(zoomLevel,gridMaxDimension),gridMinDimension));
}
...
{ coordHrDecToHrMinSec(gridLxRad*3.82);
  coordDeclDecToDegMin(gridDyRad*57.3);
