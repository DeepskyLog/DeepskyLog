...
{ var d=Math.round((gridActualDimension-(0.02*vMag)-1)*1.5);
  if(d<=1) d=1;
...
{ if(diam2==0) diam2=diam1;
  gridDrawRectangleCWH(Lhr,Ddeg,diam1,diam1,pa,'',seen);
  gridDrawEllipseTiltObject(Lhr,Ddeg,(diam1),(diam1),pa,nm,seen,object);
}
function astroDrawDRKNBObject(Lhr,Ddeg,diam1,diam2,pa,nm,seen,object)
{ if(diam2==0) diam2=diam1;
  gridDrawRectangleCWHObject(Lhr,Ddeg,diam1,diam1,pa,nm,seen,object);
}
function astroDrawDRKNBObjectInterval(Lhr,Ddeg,diam1,diam2,pa,nm,seen,object,interval)
{ if(diam2==0) diam2=diam1;
...
{ jg.setFont("verdana,geneva,helvetica,sans-serif", "10px", Font.PLAIN);
  ...
  { if(getAstroObjectValue(i,"type")=='AASTAR1')
    { name=getAstroObjectValue(i,"nameBayer")+'&nbsp;'+getAstroObjectValue(i,"nameBayer2")+'&nbsp;'; 
      ...
      astroDrawDRKNBObjectInterval(getAstroObjectValue(i,"ra"),getAstroObjectValue(i,"decl"),getAstroObjectValue(i,"diam1"),getAstroObjectValue(i,"diam2"),getAstroObjectValue(i,"pa"),(getAstroObjectValue(i,"name")),getAstroObjectValue(i,"seen"),i,10);
...
function atlasPagePanZoom(direction,factor)
...
{ if((event.keyCode>=37)&&(event.keyCode<=40)) // arrows
     atlasPagePanZoom(event.keyCode); 
  if((event.keyCode>=48)&&(event.keyCode<=57)) // 0..9
  { var $zoom=(event.keyCode-48);
    gridZoomLevel(gridMaxDimension-1-$zoom);
    atlasRedraw();
  }
...
{	return jg.fillEllipseObject(Math.round(canvasOffsetXpx+cx-(d/2)),Math.round(canvasOffsetYpx+canvasDimensionYpx-cy-(d/2)),d,d,name,object);
}
...
{ AngleDeg=((((AngleDeg*1.0)+90)%180)*0.01745);
  gridLDrad(Lhr,Ddeg); 
	x1=gridLxRad; y1=gridDyRad;
	canvasDrawEllipseTiltObjectInterval(gridCenterOffsetXpx+gridXpx(x1),gridCenterOffsetYpx+gridYpx(y1),Math.round((gridWidthXpx2*(Diam1Sec/3600)/gridSpanL)),Math.round((gridHeightYpx2*(Diam2Sec/3600)/gridSpanD)),AngleDeg,nameText,seen,object,interval);
}
...
{ gridInitScaleFixed(gridL0rad/Math.PI*12,gridD0rad/Math.PI*180,Math.max(Math.min(zoomLevel,gridMaxDimension),gridMinDimension));
}
