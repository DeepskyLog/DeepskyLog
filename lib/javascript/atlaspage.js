...
{ for(var i=0;i<conBoundries.length;i++)
   if(gridDrawLineLD(conBoundries[i]['ra0'], conBoundries[i]['decl0'], conBoundries[i]['ra1'], conBoundries[i]['decl1']))
	 { if(conBoundries[i]['con0pos']=="L")
	     canvasDrawLabel(canvasBkGroundColor, constellationColor, conBoundries[i]['con0'], ((canvasX1px+canvasX2px)/2)-(fontSize1b*3)-5, (canvasY1px+canvasY2px)/2, (fontSize1b*3), fontSize1a, 'left');
     if(conBoundries[i]['con0pos']=="R")
	     canvasDrawLabel(canvasBkGroundColor, constellationColor, conBoundries[i]['con0'], ((canvasX1px+canvasX2px)/2)+5, (canvasY1px+canvasY2px)/2, (fontSize1b*3), fontSize1a, 'left');
 	 }
}
function astroDrawConstellations()
{ if(conBoundries.length == 0)
    astroGetConstellationBoundries();
  else
...
function astroGetConstellationBoundries()
{ canvasreDrawNamedLabel(coordBkGroundColor,coordColor,'Fetching constellation boundries',5,canvasDimensionYpx-20,500,16,'left','infoline');
  ...
  { if(xmlhttp.readyState==4)
    { var xmlDoc=xmlhttp.responseXML.documentElement;
      constellationObjects=xmlDoc.getElementsByTagName("boundry");
      for(var i=0;(i<constellationObjects.length);i++)
      { var conBoundry=new Array();
        conBoundry['ra0']=getConstellationBoundryValue(i,'ra0');
        conBoundry['decl0']=getConstellationBoundryValue(i,'decl0');
        conBoundry['ra1']=getConstellationBoundryValue(i,'ra1');
        conBoundry['decl1']=getConstellationBoundryValue(i,'decl1');
        conBoundry['con0']=getConstellationBoundryValue(i,'con0');
        conBoundry['con0pos']=getConstellationBoundryValue(i,'con0pos');
        conBoundry['con1']=getConstellationBoundryValue(i,'con1');
        conBoundry['con1pos']=getConstellationBoundryValue(i,'con1pos');
        conBoundries[conBoundries.length]=conBoundry;
      }
      astroDrawConBoundries();
    }
  };
  var url="ajaxinterface.php";
  url+="?instruction=getConstellationBoundries";
  url+="&lLhr="+gridlLhr;
  url+="&dDdeg="+griddDdeg;
  url+="&rLhr="+gridrLhr;
  url+="&uDdeg="+griduDdeg;
...
{ canvasDimensionXpx=((theClientWidth-divOffsetXpx-divOffsetX2px)-(2*canvasOffsetXpx)-2);
  canvasDimensionYpx=((theClientHeight-divOffsetYpx-divOffsetY2px)-(2*canvasOffsetYpx)-2);
  cnv=document.getElementById(canvas);
  if(!cnv) return false;
    jg=new jsGraphics(cnv);
  return 1;
