...
{ var d=Math.max(Math.round((gridActualDimension-(0.02*Mag)-1.5)*1.5),1);
  jg.setColor(starColor);
...
{ astroDrawStarsArr(0,astroObjectsArr.length,99);
  ...
  canvasreDrawNamedLabel(coordBkGroundColor,coordColor,atlasPageDone,5,canvasDimensionYpx-20,900,16,'left','infoline');
  jg.paint();
...
{ jg.setFont("Lucida Console", fontSize1a+"px", Font.PLAIN);
  ...
  { if(astroObjectsArr[i]["type"]&&(astroObjectsArr[i]["type"]!='AASTAR1')&&
    ...
    { if(astroObjectsArr[i]["type"]=='AA1STAR')
        astroDrawStarObject(astroObjectsArr[i]["ra"],astroObjectsArr[i]["decl"],astroObjectsArr[i]["mag"]*100,astroObjectsArr[i]["name"],i);
      else if(astroObjectsArr[i]["type"]=='AA2STAR')
        astroDrawStarxObject(astroObjectsArr[i]["ra"],astroObjectsArr[i]["decl"],astroObjectsArr[i]["mag"]*100,astroObjectsArr[i]["name"],i);
...
{ canvasRedraw();
  ...
  gridShowInfo();
  atlasDrawLegend();
  canvasDrawNamedLabel(coordBkGroundColor,coordColor,atlasPageDone,5,canvasDimensionYpx-20,900,16,'left','infoline');
  jg.paint();
...
{ if((document.activeElement.id=='quickpickobject')||(document.activeElement.id=='deepskylog_id')||(document.activeElement.id=='passwd'))
  ...
  { location.href= 'atlas.pdf.php?ra='+atlaspagerahr+'&decl='+atlaspagedecldeg+
                            '&zoom='+gridActualDimension+
                            '&dsos='+atlasmagnitude+'&stars='+starsmagnitude+'&object='+theobject;
  }
...
{ coordHrDecToHrMinSec(gridLxRad*f12OverPi);
  ...
  return sprintf('%02d',dsl_hr)+'h'+sprintf('%02d',dsl_min)+'m'+sprintf('%02d',dsl_sec)+'s,'+sprintf('%02s',dsl_deg)+'&deg;'+sprintf('%02d',dsl_amn)+'\'';
}
// jg functions
