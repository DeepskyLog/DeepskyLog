...
{ astroDrawStarsArr(0,astroObjectsArr.length,99);
  ...
  { astroGetObjectsMagnitude((getObjectsLimit==7?-999999:getObjectsLimit),getObjectsLimit+1);
  }
}
...
{ Lhr1=Lhr1*1.0;
  ...
  var retval = 0;
  var ds = 0.0+Math.max(Math.ceil(100*Math.abs(x1-x2) / gridSpanLrad * fPiOver12),Math.ceil(100*Math.abs(y1-y2) / gridSpanDrad * fPiOver180));
  for(var i=0.0;i<ds;)
