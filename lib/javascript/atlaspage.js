...
var coordLineColor         ='#DD0000';                                          // Coordinate grid line colors
...
var gridDimensions=new Array(
  new Array(180,80.00,6.000,5),                                                 // FoV, L grid distance in deg, D grid distance in deg, default limiting star magnitude level for this zoom level 
  new Array(150,60.00,4.000,5),
  new Array(120,50.00,3.000,5),
...
function astroDrawGX(Lhr,Ddeg,diam1,diam2,pa, nm)
{ jg.setColor(starColor);
  gridDrawEllipseTilt(Lhr,Ddeg,diam1,diam2,pa,nm);
}
function astroDrawGX2(Lhr,Ddeg,diam1,diam2,pa, nm)
{ jg.setColor(starColor);
  gridDrawEllipseTilt2(Lhr,Ddeg,diam1,diam2,pa,nm);
}
function astroDrawOC(Lhr,Ddeg,diam,nm)
{ jg.setColor(starColor);
  gridDrawEllipseTilt(Lhr,Ddeg,(diam),(diam),0,nm);
}
...
{ //alert(astroObjects.length);
  ...
  { //alert(urldecode(getAstroObjectValue(i,"name")));
    if(i==153) astroDrawGX2(getAstroObjectValue(i,"ra"),getAstroObjectValue(i,"decl"),getAstroObjectValue(i,"diam1"),getAstroObjectValue(i,"diam2"),getAstroObjectValue(i,"pa"),urldecode(getAstroObjectValue(i,"name")));
      ...
      astroDrawOC(getAstroObjectValue(i,"ra"),getAstroObjectValue(i,"decl"),getAstroObjectValue(i,"diam1"),urldecode(getAstroObjectValue(i,"name")));
    else if(getAstroObjectValue(i,"type")=='GALXY')
      astroDrawGX(getAstroObjectValue(i,"ra"),getAstroObjectValue(i,"decl"),getAstroObjectValue(i,"diam1"),getAstroObjectValue(i,"diam2"),getAstroObjectValue(i,"pa"),urldecode(getAstroObjectValue(i,"name")));
    else if(getAstroObjectValue(i,"type")!='GALXY')
      astroDrawGX(getAstroObjectValue(i,"ra"),getAstroObjectValue(i,"decl"),getAstroObjectValue(i,"diam1"),getAstroObjectValue(i,"diam2"),getAstroObjectValue(i,"pa"),urldecode(getAstroObjectValue(i,"name")));
  jg.paint();
  }
  //astroDrawGX(atlaspagerahr,atlaspagedecldeg,1800,600,45,"David");
  jg.paint();
}
...
{ gridInitScale(atlaspagerahr,atlaspagedecldeg,4);
  atlasRedraw();
}
function atlasPageZoom2BtnFn()
{ gridInitScale(atlaspagerahr,atlaspagedecldeg,3);
  atlasRedraw();
}
...
{ x=event.clientX;
  y=event.clientY+1;
	...
	{	document.getElementById("myDiv").style.cursor='default';
    canvasDrawLabel(coordBkGroundColor,coordColor,"          ",0,-17,150,16,'center');
	}
	jg.paint();
}
...
function canvasDrawEllipseTilt(cx,cy,w,h,angle,name)
{	angle=-angle;
  jg.drawEllipseTiltLimited(canvasOffsetXpx+cx,canvasOffsetYpx+canvasDimensionYpx-cy,w,h,angle,canvasOffsetXpx+gridOffsetXpx,canvasOffsetXpx+gridOffsetXpx+gridWidthXpx,canvasOffsetYpx+gridOffsetYpx+gridHeightYpx,canvasOffsetYpx+gridOffsetYpx,name);
}
function canvasDrawEllipseTilt2(cx,cy,w,h,angle,name)
{	angle=-angle;
  jg.drawEllipseTiltLimited(canvasOffsetXpx+cx,canvasOffsetYpx+canvasDimensionYpx-cy,w,h,angle,canvasOffsetXpx+gridOffsetXpx,canvasOffsetXpx+gridOffsetXpx+gridWidthXpx,canvasOffsetYpx+gridOffsetYpx+gridHeightYpx,canvasOffsetYpx+gridOffsetYpx,name);
  alert("WP 3 "+name);
}
...
{ AngleDeg=(AngleDeg+90)%180;
  gridLDrad(Lhr,Ddeg); 
	...
	canvasDrawEllipseTilt(gridCenterOffsetXpx+gridXpx(x1),gridCenterOffsetYpx+gridYpx(y1),Math.round((gridWidthXpx2*(Diam1Sec/3600)/gridSpanL)),Math.round((gridHeightYpx2*(Diam2Sec/3600)/gridSpanD)),(AngleDeg/180*Math.PI),nameText);
}
function gridDrawEllipseTilt2(Lhr,Ddeg,Diam1Sec,Diam2Sec,AngleDeg,nameText)
{ AngleDeg=(AngleDeg+90)%180;
gridLDrad(Lhr,Ddeg); 
x1=gridLxRad; y1=gridDyRad;
canvasDrawEllipseTilt2(gridCenterOffsetXpx+gridXpx(x1),gridCenterOffsetYpx+gridYpx(y1),Math.round((gridWidthXpx2*(Diam1Sec/3600)/gridSpanL)),Math.round((gridHeightYpx2*(Diam2Sec/3600)/gridSpanD)),(AngleDeg/180*Math.PI),nameText);
}
...
{ gridActualDimension=gridMaxDimension;
  ...
  //alert('Actual Dimension: '+gridActualDimension);
}
