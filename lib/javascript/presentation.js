...
function readTitleColumnData()
{ i=0;
  while(document.getElementById('C'+i))
  { titleData[i]=document.getElementById('C'+i).innerHTML;
    t=0; var tempAttributes= new Array(); var tempAttributesValues = new Array();
    while(t<document.getElementById('C'+i).attributes.length)
    { if(document.getElementById('C'+i).attributes.item(t).nodeName!="id")
      { tempAttributes[tempAttributes.length]=document.getElementById('C'+i).attributes.item(t).nodeName;
        tempAttributesValues[tempAttributesValues.length]=document.getElementById('C'+i).attributes.item(t).nodeValue;
      }
      //deveolpmentlines to check slowness of MSIE
      //if(i==0)
      //  alert(document.getElementById('C'+i).attributes.item(t).nodeName+' '+document.getElementById('C'+i).attributes.item(t).nodeValue);
      t++;
    }
    ...
    { tempColumnData[j]=document.getElementById('C'+i+'D'+j).innerHTML;
      ...
      t=0; var tempAttributes= new Array(); var tempAttributesValues = new Array();
      while(t<document.getElementById('C'+i+'D'+j).attributes.length)
      { if(document.getElementById('C'+i+'D'+j).attributes.item(t).nodeName!="id")
    	{ tempAttributes[tempAttributes.length]=document.getElementById('C'+i+'D'+j).attributes.item(t).nodeName;
    	  tempAttributesValues[tempAttributesValues.length]=document.getElementById('C'+i+'D'+j).attributes.item(t).nodeValue;
    	}
        t++;
      }    
...
{ i=0;
  ...
  { document.getElementById('C'+i).innerHTML=titleData[orderColumnSourceOrder[i].substr(1)];
    ...
    t=document.getElementById('C'+i).attributes.length-1;
    while(t>0)
    { if(document.getElementById('C'+i).attributes.item(t).nodeName!="id")
      { document.getElementById('C'+i).removeAttribute(document.getElementById('C'+i).attributes.item(t).nodeName);
      }
      t--;
    }
    t=0;
    ...
    { document.getElementById('C'+i+'D'+j).innerHTML=columnData[orderColumnSourceOrder[i].substr(1)][j];
      ...
      t=document.getElementById('C'+i+'D'+j).attributes.length-1;
      while(t>0)
      { if(document.getElementById('C'+i+'D'+j).attributes.item(t).nodeName!="id")
        { document.getElementById('C'+i+'D'+j).removeAttribute(document.getElementById('C'+i+'D'+j).attributes.item(t).nodeName);
        }
        t--;
      }
      t=0;
...
{ for(i=0;i<restoreColumnsSourceCollapsed.length;i++)
  { //check actual location if column
	...
	while(orderColumnSourceOrder[k]!=restoreColumnsSourceCollapsed[i])
	  k++;
	//empty column title
	document.getElementById('C'+k).innerHTML="<a href=\"#\" onclick=\"restoreColumn('"+restoreColumnsSourceCollapsed[i]+"');return false;\" title=\""+restoreColumnsSourceCollapsedHeaders[i]+"\">.</a>";
	j=0; 
...
{ //check actual location if column
  ...
  while(orderColumnSourceOrder[k]!=theColumn)
	k++;
  //empty column
  document.getElementById('C'+k).innerHTML="<a href=\"#\" onclick=\"restoreColumn('"+theColumn+"');return false;\" title=\""+header0+"\">.</a>";
  i=0; 
