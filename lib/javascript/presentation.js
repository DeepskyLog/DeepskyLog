...
{ //read all orders cookie
  ...
  { k=0;
    ...
    { if(orderColumns[i][0]==theColumnSource)
      { orderColumnSourceOrder[k]=orderColumns[i][1];
        k++;
      }
    }
  }
  ...
  else if(orderColumnSourceOrder.length<theMaxColNr+1)
  { cookiestring='';
    if(orderColumns)
    { for(i=0;i<orderColumns.length;i++)
        if(orderColumns[i][0]!=theColumnSource)
    ...
    i=orderColumnSourceOrder.length;
    while(i<=theMaxColNr)
    { cookiestring=cookiestring+"['"+theColumnSource+"','C"+i+"'],";	  
      orderColumnSourceOrder[i]='C'+i;
      i++;
...
{ i=0;
  while(i<orderColumnSourceOrder.length)
...
{ var cookie = new Array();
  if(getCookie('orderColumns'))
	orderColumns=eval(getCookie('orderColumns'));
  var cookiestring='';
...
{ //read all orders cookie
  var orderColumns=eval('['+newcookiestring+']');
  //filter this form's orders => orderColumnSourceOrder
  if(orderColumns)
  { k=0;
    for(i=0;i<orderColumns.length;i++)
...
{ var cookie = new Array();
  ...
  //get the cookies
  var restoreColumns=eval(getCookie('restoreColumns'));
  cookiestring='';
  //rebuild the other cookies
  if(restoreColumns)
  ...
  var exdate= new Date();
  exdate.setDate(exdate.getDate()+365);
  document.cookie="restoreColumns=["+cookiestring.substr(0,cookiestring.length-1)+"];expires="+exdate.toUTCString();	 
}
