...
/** 
 * All functions needed to enter, retrieve and adapt observation data from the database.
 * 
 * @category Utilities/Deepsky
 * @package  DeepskyLog
 * @author   DeepskyLog Developers <developers@deepskylog.be>
...
class Observations
{
      ...
      *
      * @return string The status of the import. How many observations where imported, ...
      */
    public function addCSVobservations()
    {
        global $objPresentations, $messageLines, $objObject, $objLocation, $loggedUser, $objInstrument, $objEyepiece, $objLens, $objFilter;
        ...
        if (!is_array($objects)) {
            ...
            for ($i = 0, $j = 0, $temploc = ''; $i < count($locations); $i++) {
                if ((!trim($locations[$i])) || ($temploc != trim($locations[$i])) 
                    && ($objLocation->getLocationId(trim($locations[$i]), $loggedUser) == -1)
                ) {
            ...
            for ($i = 0, $j = 0; $i < count($eyepieces); $i++) {
                if (trim($eyepieces[$i]) 
                    && (!($objEyepiece->getEyepieceObserverPropertyFromName(trim($eyepieces[$i]), $loggedUser, 'id')))
                ) {
            ...
            for ($i = 0, $j = 0; $i < count($filters); $i++) {
                if (trim($filters[$i]) 
                    && (!($objFilter->getFilterObserverPropertyFromName(trim($filters[$i]), $loggedUser, 'id')))
                ) {
            ...
            if (count($errorlist) > 0) {
                ...
                if (count($objectsMissing) > 0) {
                    ...
                    for ($i = 0; $i < count($objectsMissing); $i++) {
                        // Only show the first 10 objects missing, then add the number of other objects missing
                        if ($i == 10) {
                            $errormessage .= "<li>" . LangCSVError2b . (count($objectsMissing) - 10) . LangCSVError2c . "</li>";
                            break;
                        }
                ...
                if (count($locationsMissing) > 0) {
                    ...
                    for ($i = 0; $i < count($locationsMissing); $i++) {
                        // Only show the first 10 locations missing, then add the number of other locations missing
                        if ($i == 10) {
                            $errormessage .= "<li>" . LangCSVError2b . (count($locationsMissing) - 10) . LangCSVError3b . "</li>";
                            break;
                        }
                ...
                if (count($instrumentsMissing) > 0) {
                    ...
                    for ($i = 0; $i < count($instrumentsMissing); $i++) {
                        // Only show the first 10 instruments missing, then add the number of other instruments missing
                        if ($i == 10) {
                            $errormessage .= "<li>" . LangCSVError2b . (count($instrumentsMissing) - 10) . LangCSVError4b . "</li>";
                            break;
                        }
                ...
                if (count($filtersMissing) > 0) {
                    ...
                    for ($i = 0; $i < count($filtersMissing); $i++) {
                        // Only show the first 10 filters missing, then add the number of other filters missing
                        if ($i == 10) {
                            $errormessage .= "<li>" . LangCSVError2b . (count($filtersMissing) - 10) . LangCSVError5b . "</li>";
                            break;
                        }
                ...
                if (count($eyepiecesMissing) > 0) {
                    ...
                    for ($i = 0; $i < count($eyepiecesMissing); $i ++) {
                        // Only show the first 10 eyepieces missing, then add the number of other eyepieces missing
                        if ($i == 10) {
                            $errormessage .= "<li>" . LangCSVError2b . (count($eyepiecesMissing) - 10) . LangCSVError6b . "</li>";
                            break;
                        }
                ...
                if (count($lensesMissing) > 0) {
                    ...
                    for ($i = 0; $i < count($lensesMissing); $i++) {
                        // Only show the first 10 lenses missing, then add the number of other lenses missing
                        if ($i == 10) {
                            $errormessage .= "<li>" . LangCSVError2b . (count($lensesMissing) - 10) . LangCSVError7b . "</li>";
                            break;
                        }
                ...
                if (!in_array($i, $errorlist)) {
                    ...
                    if (trim($parts_array[$i][1]) == $username) {
                        ...
                        $locat = $objLocation->getLocationId(
                            htmlentities(trim($parts_array[$i][4]), ENT_COMPAT, "UTF-8", 0), $loggedUser
                        );
            ...
            return LangCSVMessage8 . ": " . $added . LangCSVMessage9 . ": " . count($errorlist) . LangCSVMessage10 . ": "
