...
{ if ($_POST['passwd']!=$_POST['passwd_again'])                                 // passwd and passwd_again don't match
  ...
  { if (!preg_match("/.*@.*..*/", $_POST['email']) | preg_match("/(<|>)/", $_POST['email']))
    { $entryMessage = LangValidateAccountMessage3;                              // check if email address is legal (contains @ symbol)
      $_GET['indexAction']="default_action";                                    // check if username doesn't exist yet 
    }                                                                           // pressed register button and username filled in    
    if(array_key_exists('register',$_POST)&&array_key_exists('deepskylog_id',$_POST)&&$_POST['register']&&$_POST['deepskylog_id']) 
    { if(!$objObserver->getObserverName($_POST['deepskylog_id']))               // user doesn't exist yet
      { $objDatabase->addObserver($_POST['deepskylog_id'],$_POST['name'],$_POST['firstname'],$_POST['email'],md5($_POST['passwd']));
