...
<table class="table table-sm">
    ...
    <tr>
        <td>{{ _i("Diameter") }}</td>
        @auth
        <td>{{ Auth::user()->showInches ? (number_format($instrument->diameter / 25.4, 2, '.', ',')) . ' ' . _i('inch') : $instrument->diameter . ' ' . _i('mm')}}
        </td>
        @endauth
        @guest
    ...
    <tr>
        ...
        @auth
        <td>{{ Auth::user()->showInches ? (number_format($instrument->fd * $instrument->diameter / 25.4, 2, '.' ,',')) . ' ' . _i('inch') : $instrument->fd * $instrument->diameter . ' ' . _i('mm')}}
            (F/{{ $instrument->fd }})</td>
        @endauth
        @guest
