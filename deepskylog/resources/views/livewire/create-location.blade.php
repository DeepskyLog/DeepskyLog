                ...
                <x-card>
                    <form role="form" method="POST" wire:submit.prevent="save">
                        ...
                        <div class="col-span-6 sm:col-span-5">
                            ...
                            <x-input name="name" label="{{ __('Name of the location') }}" type="text"
                                wire:model.live="name" class="mt-1 block w-full" required maxlength="255"
                                value="{{ old('name') }}" />

                            <!-- Leaflet.js Map and Search -->
                            <div class="mt-4">
                                <div id="map" class="w-full h-96 mt-4 rounded" style="z-index:1;" wire:ignore>
                                </div>
                            </div>


                            <input type="hidden" name="latitude" wire:model.live="latitude"
                                value="{{ old('latitude') }}" />

                            <input type="hidden" name="longitude" wire:model.live="longitude"
                                value="{{ old('longitude') }}" />

                            <input type="hidden" name="elevation" wire:model.live="elevation"
                                value="{{ old('elevation') }}" />

                            <input type="hidden" name="country" wire:model.live="country"
                                value="{{ old('country') }}" />

                            <input type="hidden" name="timezone" wire:model.live="timezone"
                                value="{{ old('timezone') }}" />

                            <!-- SQM, NELM and Bortle on one line -->
                            <div class="mt-4 flex flex-col gap-3 md:flex-row">
                                <div class="flex-1">
                                    <x-input name="sqm" label="{{ __('SQM (mag/arcsec²)') }}" type="number"
                                        step="0.01" min="15" max="22" wire:model.live="sqm"
                                        class="mt-1 block w-full" value="{{ old('sqm') }}" />
                                </div>

                                <div class="flex-1">
                                    <x-input name="nelm" label="{{ __('NELM') }}" type="number" step="0.1"
                                        min="0" max="8" wire:model.live="nelm" class="mt-1 block w-full"
                                        value="{{ old('nelm') }}" />
                                </div>

                                <div class="flex-1">
                                    <x-select id="bortle" name="bortle" label="{{ __('Bortle') }}"
                                        wire:model.live="bortle" class="mt-1 block w-full h-10"
                                        x-on:selected="$wire.set('bortle', $event.detail.value)" :options="[
                            ...
                            <x-toggle name="hidden" label="{{ __('Hide the exact location for other users') }}"
                                wire:model="hidden" class="mt-2" id="hidden" />
                            ...
                            <div class="col-span-6 sm:col-span-5" wire:ignore>
                                <textarea wire:model.live="description" class="h-48 min-h-fit" name="description" id="description"></textarea>
                            </div>

                            <div class="mt-5">
                                <x-input type="file" label="{!! __('Upload image') !!}" wire:model="photo" />
                                ...
                                @if ($update)
                                    @if ($location->picture)
                                        <img alt="{{ __('Picture of location') }}" class="mt-2 h-40 w-40 object-cover"
                            ...
                            @if ($update)
                                <x-button class="mt-5" type="submit" secondary
                                    label="{{ __('Update location') }}" />
                            @else
                                <x-button class="mt-5" type="submit" secondary
                                    label="{{ __('Add new location') }}" />
                            @endif
...
@push('scripts')
    ...
    <script>
        ...
        function initLeafletMap() {
            ...
            // console.log removed: Initializing Leaflet map
