<tbody id="dsl-aladin-preview-info" x-data @dsl-aladin-updated.window="$wire.call('recalculate', $event.detail)" x-init="(function(){ try {
    ...
    @if($last_error)
        <tr><td colspan="2" class="text-xs text-red-400 mb-2">Last error: {{ $last_error }}</td></tr>
    @endif
    <tr>
        <td class="pr-4 font-medium">
            <div class="inline-flex items-center space-x-2">
                ...
                <div x-data="{ openCRInfo: false }" class="relative inline-block">
                    <button @click.prevent="openCRInfo = !openCRInfo" @keydown.escape="openCRInfo = false" :aria-expanded="openCRInfo.toString()" aria-haspopup="true" type="button" class="ml-1 text-gray-400 hover:text-gray-200 focus:outline-none" aria-label="Contrast reserve info">
                        {{-- simple info SVG --}}
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                            <path fill-rule="evenodd" d="M18 10A8 8 0 11 2 10a8 8 0 0116 0zm-9-4a1 1 0 112 0 1 1 0 01-2 0zM9 9a1 1 0 012 0v4a1 1 0 11-2 0V9z" clip-rule="evenodd" />
                        </svg>
                    </button>

                    <div x-show="openCRInfo" x-cloak @click.outside="openCRInfo = false" x-transition class="absolute z-10 left-0 mt-2 w-72 p-3 bg-gray-800 text-sm text-gray-100 rounded shadow-lg" data-dsl-no-overlay-hide="true">
                        ...
                        <ul class="text-xs space-y-2">
                            @php
                                ...
                                $rows = [
                                    ...
                                    ['color' => 'bg-orange-400', 'label' => __('Quite difficult'), 'value' => '> 0.35'],
                            ...
                            @foreach($rows as $r)
                                <li class="flex items-start">
                                    <span class="inline-block w-2 h-2 rounded-full {{ $r['color'] }} mr-2 mt-1"></span>
        ...
        <td>
            ...
            @else
                @php
                    ...
                    $crClass = 'text-white';
                    if ($crCat === 'very_easy') { $crClass = 'text-green-400'; }
                    elseif ($crCat === 'easy') { $crClass = 'text-green-700'; }
                    elseif ($crCat === 'quite_difficult') { $crClass = 'text-orange-400'; }
                    elseif ($crCat === 'difficult') { $crClass = 'text-orange-700'; }
                    elseif ($crCat === 'questionable') { $crClass = 'text-gray-300'; }
                    elseif ($crCat === 'not_visible') { $crClass = 'text-gray-600'; }
                ...
                <div x-data="{ openCR: false }" class="inline-block relative">
                    <button @click.prevent="openCR = !openCR" @keydown.escape="openCR = false" :aria-expanded="openCR.toString()" aria-haspopup="true" type="button" class="focus:outline-none {{ $crClass }} font-medium">
                        ...
                        <div x-show="openCR" x-cloak @click.outside="openCR = false" x-transition class="absolute z-10 left-0 mt-2 w-80 p-3 bg-gray-800 text-sm text-gray-100 rounded shadow-lg" data-dsl-no-overlay-hide="true">
                            <div class="text-sm mb-2">{{ __('contrast.reserve.summary', ['value' => $contrast_reserve, 'category' => $categoryText]) }}</div>
                            <div class="text-xs text-gray-300 mb-1"><strong>{{ __('Location') }}:</strong> {{ $contrast_used_location ?? __('Unknown') }}@if(!empty($contrast_used_sqm) || !empty($contrast_used_nelm))
                                    <span class="text-gray-400">(@if(!empty($contrast_used_sqm)) SQM: {{ $contrast_used_sqm }}@endif@if(!empty($contrast_used_sqm) && !empty($contrast_used_nelm)) / @endif@if(!empty($contrast_used_nelm)) NELM: {{ $contrast_used_nelm }}@endif)</span>
                                @endif</div>
                            <div class="text-xs text-gray-300"><strong>{{ __('Instrument') }}:</strong> {{ $contrast_used_instrument ?? __('Unknown') }}</div>
                        </div>
                </div>
            @endif
        </td>
    ...
    <tr>
        ...
        <td>
            @if(!empty($optimum_detection_magnification))
                ...
                @endphp
                <div>{{ $optimum_detection_magnification }}x - {!! implode(', ', $eps) !!}</div>
            @else
                {{ __('Unknown') }}
            @endif
        </td>
