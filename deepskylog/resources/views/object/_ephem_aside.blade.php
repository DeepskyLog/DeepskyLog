    ...
    <div class="text-sm space-y-2">
        @auth
            <div>
                ...
                <div class="mt-1">
                    <input id="ephem-date-input" type="date" class="bg-gray-900 text-white px-2 py-1 rounded text-sm w-full"
                        value="{{ isset($ephemerides['date']) ? $ephemerides['date'] : \Carbon\Carbon::now()->toDateString() }}" />
                </div>
            </div>
            ...
            <div>
                ...
                <div class="mt-1">
                    <div class="flex items-center gap-2">
                        <div class="w-12 h-12 bg-black rounded flex items-center justify-center text-sm text-gray-300"
                            id="ephem-moon-image">
                        ...
                        <div class="text-sm">
                            <div>{{ __('Illumination') }}: <span
                                    class="font-mono">{{ isset($ephemerides['moon_illuminated']) ? $ephemerides['moon_illuminated'] * 100 . '%' : '—' }}</span>
                            </div>
                            <div>{{ __('Next new moon') }}: <span
                                    class="font-mono">{{ $ephemerides['next_new_moon'] ?? '—' }}</span></div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="mt-1 font-mono">{{ $ephemerides['rising'] ?? '—' }} / {{ $ephemerides['setting'] ?? '—' }}
                </div>
            </div>

            <div>
                <div class="text-gray-400 text-xs">{{ __('Sun rise / set') }}</div>
                <div class="mt-1 font-mono">{{ $ephemerides['sunrise'] ?? '—' }} / {{ $ephemerides['sunset'] ?? '—' }}
                </div>
            </div>

            <div>
                <div class="text-gray-400 text-xs">{{ __('Nautical twilight') }}</div>
                <div class="mt-1 font-mono">{{ $ephemerides['nautical_twilight_begin'] ?? '—' }} /
                    {{ $ephemerides['nautical_twilight_end'] ?? '—' }}</div>
            </div>

            <div>
                <div class="text-gray-400 text-xs">{{ __('Astronomical twilight') }}</div>
                <div class="mt-1 font-mono">{{ $ephemerides['astronomical_twilight_begin'] ?? '—' }} /
                    {{ $ephemerides['astronomical_twilight_end'] ?? '—' }}</div>
            </div>
        @endauth
