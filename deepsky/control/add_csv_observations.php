...
{ $objects = array_values($objects);
  ...
  { $objectsquery=$objObject->getExactObject($objects[$i]);
  ...
  for($i=0,$j=0;$i<count($lenses);$i++)
    if($lenses[$i]&&($lns->getLensId($lenses[$i],$_SESSION['deepskylog_id'])==-1))
    ...
    { $observername = $objObserver->getFirstname($parts_array[$i][1]). " ".$objObserver->getObserverName($parts_array[$i][1]);
      ...
      { $instrum = $inst->getInstrumentId($parts_array[$i][5], $_SESSION['deepskylog_id']);
        ...
        $obsid=$objObservation->addDSObservation($correctedObjects[$i-1],$_SESSION['deepskylog_id'],$instrum,$locat,$date,$time,htmlentities($parts_array[$i][13]),$parts_array[$i][9],$parts_array[$i][10],$parts_array[$i][11],$parts_array[$i][12]);
				if ($parts_array[$i][6] != "")
				  $objObservation->setEyepieceId($obsid, $eyep->getEyepieceId($parts_array[$i][6], $_SESSION['deepskylog_id']));
				if ($parts_array[$i][7] != "")
					$objObservation->setFilterId($obsid, $filt->getFilterId($parts_array[$i][7], $_SESSION['deepskylog_id']));
				if ($parts_array[$i][8] != "")
					$objObservation->setLensId($obsid, $lns->getLensId($parts_array[$i][8], $_SESSION['deepskylog_id']));
      }
    }
    // upload successful
