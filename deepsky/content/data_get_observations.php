	     ...
	     (array_key_exists('maxseeing',$_GET) && $_GET['maxseeing']!="")) // at least 1 field to search on 
{
  ...
  if(array_key_exists('seen', $_GET) && $_GET['seen'])
   ...
   $querytotal = array("object" => $object,
	                "catalog" => $catalogue,
									"number" => $number,
                  "observer" => $observer,
                  "instrument" => $instrument,
   ...
   if($validQobs)
	 {
	   $obs=$_SESSION['Qobs'];
		 $total=$_SESSION['QobsTotal'];
	 }
	 else
	 { $obs = $objObservation->getObservationFromQuery($query,$sort,$sortdirection,$seenpar); 
     $total = $objObservation->getObservationFromQuery($querytotal,"","ASC", $seenpar); 
     $_SESSION['QobsParams']=$query;
		 $_SESSION['Qobs']=$obs;
		 $_SESSION['QobsSort']=$sort;
		 $_SESSION['QobsSortDirection']=$sortdirection;
		 $_SESSION['QobsTotal']=$total;
	 }
