...
if(array_key_exists('object', $_GET) && $_GET['object'])
{
  ...
  echo("&nbsp;&nbsp;" . LangViewObservationField34 . "
            <input type=\"text\" class=\"inputfield\" maxlength=\"4\" name=\"sqm\" size=\"4\"");
  if(array_key_exists('newObsSqm', $_SESSION) && array_key_exists('newObsSavedata', $_SESSION) && ($_SESSION['newObsSavedata'] == "yes" && $_SESSION['newObsSqm'] != ""))
  echo (" value=\"" . sprintf("%2.1f", $_SESSION['newObsSqm']) . "\" />");
  else
  echo (" value=\"\" />");
  echo("</td><td></td>");
  // SEEING
