...
function new_listdatacsv()
{
    ...
    if ($objList->checkList($_SESSION['listname']) == 2) {
        echo _('This form gives you the possibility to add different objects at once using a CSV file (separated value by semicolumn). The form makes it also possible to easily add objects from another database to your DeepskyLog list.') . '<br />';
        echo _('The CSV file has the following structure and does not contain the header:') . '<br /><br />';
